...
const UserCreate: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://olymp-admin-v2/api/user/create', {
          headers: {
            'Authorization': token
          },
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: value // убрано преобразование в число
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://olymp-admin-v2/api/user/store', formData, {
        headers: {
          'Authorization': token,
    ...
    <div className="container user-form mt-4">
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          ...
          <li className="breadcrumb-item active">Создание нового пользователя</li>
        </ol>
      </nav>

      <h1 className="mb-4">Создание нового пользователя</h1>
